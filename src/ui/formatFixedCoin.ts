...
const SATOSHIS_PER_FIX = BigInt(100_000_000);

export const formatFixedCoinAmount = (
  ...
  if (
    ...
    throw new Error('Invalid FIX fraction digit range.');
  }
  const normalized =
  ...
  const whole = groupFrenchDigits((absolute / SATOSHIS_PER_FIX).toString());
  const allFraction = (absolute % SATOSHIS_PER_FIX).toString().padStart(8, '0');
