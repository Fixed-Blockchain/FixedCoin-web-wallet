...
describe('address QR', () => {
  it('encodes the raw validated FixedCoin address into a deterministic square path', () => {
  ...
  it('rejects non-FixedCoin or private-address placeholders before encoding', () => {
    expect(() => createAddressQr('not-an-address')).toThrow();
    expect(() =>
      createAddressQr('fix1zqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq'),
