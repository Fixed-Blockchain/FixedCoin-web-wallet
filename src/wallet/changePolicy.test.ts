...
import { instantiateFixedCoinWasmCrypto } from '../crypto/wasmAbi';
...
describe('Bitcoin Core recipient-based change policy', () => {
  ...
  beforeAll(async () => {
    const wasm = await instantiateFixedCoinWasmCrypto(
      await readFile(resolve('public/wasm/fixedcoin_wallet_crypto_web.wasm')),
  ...
  it('never selects an unavailable family or a foreign-network address', () => {
    ...
    // FIX shares Bitcoin's Legacy/P2SH prefixes; those addresses cannot be
    // distinguished by network from their encoding alone.
    expect(
      changeAccountForWallet(['b9BsYdi14nFc7tiYez85YNtDaJGx36xn7'], HD),
    ).toBe('legacy');
  });
});
