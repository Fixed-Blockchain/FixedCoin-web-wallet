...
export const FIX_SIGNER_NETWORK = {
  bech32: 'fix',
  pubKeyHash: 0x01,
  scriptHash: 0x00,
  wif: 0x80,
...
export const parseFixedCoinAmountToSats = (amount: string) => {
...
const validateOwnerMaterial = (owner: ScannedAddress) => {
  ...
  try {
    expectedScript = scriptPubKeyForFixedCoinAddress(owner.address);
  } catch (caught) {
...
const normalizeSendOutputs = (outputs: readonly TransparentSendOutput[]) => {
  ...
  return outputs.map((output) => {
    ...
    try {
      scriptPubKeyForFixedCoinAddress(address);
    } catch (caught) {
...
const selectSpend = ({
  ...
  if (optimizeSelection && inputs.length > 1) {
    ...
    // winner: at most 25% extra, capped at 1,000 satoshis (0.00001 FIX).
  ...
  for (const output of best.outputs) {
    if ('address' in output)
      tx.addOutputAddress(output.address, output.amount, FIX_SIGNER_NETWORK);
...
export const calculateMaxTransparentSendAmount = async (
  ...
  try {
    scriptPubKeyForFixedCoinAddress(candidateOutputs[targetIndex]!.address);
  } catch (caught) {
    throw new TransparentSendError('recipient-address-invalid', caught);
  }
...
const outputScriptHex = (output: Record<string, unknown>) => {
  ...
  try {
    return bytesToHex(scriptPubKeyForFixedCoinAddress(output.address));
  } catch (caught) {
