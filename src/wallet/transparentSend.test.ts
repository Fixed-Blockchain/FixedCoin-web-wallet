...
describe('transparent send selection', () => {
  ...
  beforeAll(async () => {
    wasm = await instantiateFixedCoinWasmCrypto(
      await readFile(
  ...
  it.each([
    ...
    async (...order) => {
      ...
      // Taproot costs 260, Bech32 282, P2SH 350 and Legacy 462 satoshis.
  ...
  it.each([29, 30])(
    'caps the premium at 1000 satoshis when fees rise (rate: %s)',
    async (rate) => {
      const snapshot = selectionSnapshot(
      ...
      // Both fit the relative margin; the P2SH premium is 986 or 1020 satoshis.
      expect(signed.walletInputs[0]!.valueSats).toBe(
  ...
  it('parses FIX amounts without floating point arithmetic', () => {
    expect(parseFixedCoinAmountToSats('1')).toBe(BigInt(100_000_000));
    expect(parseFixedCoinAmountToSats('0,12345678')).toBe(BigInt(12_345_678));
    expect(parseFixedCoinAmountToSats('99 993,87651555')).toBe(
      BigInt('9999387651555'),
    );
    expect(() => parseFixedCoinAmountToSats('0.000000001')).toThrow(
      expect.objectContaining({ code: 'invalid-amount-format' }),
    );
    expect(() => parseFixedCoinAmountToSats('0.00000001')).toThrow(
      expect.objectContaining({ code: 'amount-below-dust' }),
    );
    expect(() => parseFixedCoinAmountToSats('9'.repeat(100_000))).toThrow(
      expect.objectContaining({ code: 'invalid-amount-format' }),
    );
    expect(() => parseFixedCoinAmountToSats('90071992.54740992')).toThrow(
      expect.objectContaining({ code: 'invalid-amount-format' }),
    );
  });
  ...
  it('rejects invalid recipients and unconfirmed-only funds before signing', async () => {
    ...
    await expect(
      estimateTransparentMultiSend({
        snapshot,
        outputs: [{ address: 'not-fixedcoin', amountSats: BigInt(25_000_000) }],
        changeAddress: internal.address,
      }),
