...
type WasmCryptoExports = WebAssembly.Exports & {
  ...
  fixedcoin_wallet_crypto_alloc(length: number): number;
  fixedcoin_wallet_crypto_free(pointer: number): void;
  fixedcoin_wallet_crypto_free_bytes(pointer: number, length: number): void;
  fixedcoin_wallet_crypto_invoke(operation: number, request: number): number;
};
...
export class FixedCoinCryptoError extends Error {
  constructor(
    public readonly code: string,
    message: string,
  ) {
    super(message);
    this.name = 'FixedCoinCryptoError';
  }
}
...
function isWasmCryptoExports(value: WebAssembly.Exports): value is WasmCryptoExports {
  ...
  return (
    ...
    typeof candidate.fixedcoin_wallet_crypto_alloc === 'function' &&
    typeof candidate.fixedcoin_wallet_crypto_free === 'function' &&
    typeof candidate.fixedcoin_wallet_crypto_free_bytes === 'function' &&
    typeof candidate.fixedcoin_wallet_crypto_invoke === 'function'
  );
}
...
function readNullTerminatedUtf8(memory: WebAssembly.Memory, pointer: number): string {
  if (!Number.isSafeInteger(pointer) || pointer <= 0) {
    throw new FixedCoinCryptoError('INVALID_WASM_RESPONSE', 'The WASM core returned an invalid pointer.');
  }
  ...
  if (end === upperBound) {
    throw new FixedCoinCryptoError(
...
export class FixedCoinWasmCrypto {
  ...
  invoke<T>(operation: string, request: unknown): T {
    if (!/^[A-Za-z][A-Za-z0-9]{0,63}$/u.test(operation)) {
      throw new FixedCoinCryptoError('INVALID_OPERATION', 'Invalid cryptographic operation name.');
    }
    ...
    if (requestBytes.length > MAX_WASM_STRING_BYTES) {
      requestBytes.fill(0);
      throw new FixedCoinCryptoError('REQUEST_TOO_LARGE', 'Cryptographic request exceeds size policy.');
    }

    const operationPointer = this.exports.fixedcoin_wallet_crypto_alloc(operationBytes.length);
    const requestPointer = this.exports.fixedcoin_wallet_crypto_alloc(requestBytes.length);
    if (!operationPointer || !requestPointer) {
      if (operationPointer) {
        this.exports.fixedcoin_wallet_crypto_free_bytes(operationPointer, operationBytes.length);
      }
      if (requestPointer) {
        this.exports.fixedcoin_wallet_crypto_free_bytes(requestPointer, requestBytes.length);
      }
      operationBytes.fill(0);
      requestBytes.fill(0);
      throw new FixedCoinCryptoError('WASM_ALLOCATION_FAILED', 'The WASM core could not allocate memory.');
    }
    ...
    try {
      ...
      responsePointer = this.exports.fixedcoin_wallet_crypto_invoke(operationPointer, requestPointer);
      ...
      if (!envelope.ok) {
        throw new FixedCoinCryptoError(envelope.error.code, envelope.error.message);
      }
      ...
      this.exports.fixedcoin_wallet_crypto_free_bytes(operationPointer, operationBytes.length);
      this.exports.fixedcoin_wallet_crypto_free_bytes(requestPointer, requestBytes.length);
      if (responsePointer) this.exports.fixedcoin_wallet_crypto_free(responsePointer);
    }
  }
}

export async function instantiateFixedCoinWasmCrypto(
  source: BufferSource,
  entropySource: WasmEntropySource = fillWithWebCrypto,
): Promise<FixedCoinWasmCrypto> {
  ...
  if (!isWasmCryptoExports(instance.exports)) {
    throw new FixedCoinCryptoError(
      'INVALID_WASM_MODULE',
      'The WASM module does not expose the expected FixedCoin crypto ABI.',
    );
  }
  state.exports = instance.exports;
  return new FixedCoinWasmCrypto(instance.exports);
}
