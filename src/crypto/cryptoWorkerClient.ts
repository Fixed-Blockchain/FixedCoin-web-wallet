import { FixedCoinCryptoError } from './wasmAbi';
...
export class CryptoWorkerClient {
  ...
  private readonly handleMessage = (event: MessageEvent<unknown>) => {
    ...
    if (response.ok) {
      ...
      pending.reject(new FixedCoinCryptoError(response.error.code, response.error.message));
    }
  };
  ...
  async request<T extends CryptoWorkerCommand['type']>(
    ...
    return new Promise<CryptoWorkerResultByCommand[T]>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new FixedCoinCryptoError('CRYPTO_WORKER_TIMEOUT', 'Cryptographic operation timed out.'));
      }, this.timeoutMs);
      this.pending.set(id, {
  ...
  dispose(): void {
    this.rejectAll(new FixedCoinCryptoError('CRYPTO_WORKER_TERMINATED', 'The cryptographic worker closed.'));
    this.destroyWorker();
  }
