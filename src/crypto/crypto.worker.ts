...
const WASM_URL = new URL('/wasm/fixedcoin_wallet_crypto_web.wasm', workerScope.location.origin);
...
let cryptoPromise: Promise<FixedCoinWasmCrypto> | undefined;
let activeSession: ActiveSession | undefined;

const BIP39_VAULT_CONTEXT = 'fixedcoin-wallet-web:bip39-session:v1';
const PRIVATE_KEY_VAULT_CONTEXT = 'fixedcoin-wallet-web:private-key-session:v1';
const emailVaultContext = (email: string) =>
  `fixedcoin-wallet-web:email-session:v1:${email}`;

async function loadCrypto(): Promise<FixedCoinWasmCrypto> {
  cryptoPromise ??= (async () => {
    ...
    return instantiateFixedCoinWasmCrypto(bytes);
  })();
...
function assertSession(sessionId: string): ActiveSession {
  if (!activeSession || activeSession.id !== sessionId) {
    throw new FixedCoinCryptoError('SESSION_LOCKED', 'The wallet session is locked or expired.');
  }
...
function normalizeMnemonic(input: string): { mnemonic: string; wordCount: 12 | 24 } {
  ...
  if (count !== 12 && count !== 24) {
    throw new FixedCoinCryptoError('INVALID_MNEMONIC', 'A BIP39 phrase must contain 12 or 24 words.');
  }
...
async function openHdSession(
  ...
  if (source === 'email-credentials') {
    if (!email || emailPassword === undefined) {
      throw new FixedCoinCryptoError('EMAIL_REQUIRED', 'Wallet email is unavailable.');
    }
...
function cryptoRandomId(): string {
  if (typeof crypto.randomUUID !== 'function') {
    throw new FixedCoinCryptoError('ENTROPY_UNAVAILABLE', 'Secure session identifiers are unavailable.');
  }
...
workerScope.addEventListener('message', (event) => {
  ...
  void execute(command)
    ...
    .catch((error: unknown) => {
      const code = error instanceof FixedCoinCryptoError ? error.code : 'CRYPTO_WORKER_ERROR';
