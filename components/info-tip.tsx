  ...
  return (
    ...
    >
      <button
        ...
        aria-expanded={open}
        aria-label={label}
        className="grid size-8 place-items-center rounded-full border border-white/10 bg-white/[0.035] text-stone-500 outline-none transition-all duration-200 hover:-transtone-y-0.5 hover:border-amber-300/30 hover:bg-amber-300/10 hover:text-amber-200 focus-visible:border-amber-300/50 focus-visible:ring-4 focus-visible:ring-amber-300/10"
        onBlur={(event) => {
      ...
      <span
        ...
        role="tooltip"
        aria-hidden={!open}
        className={`fixed z-50 max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl border border-amber-200/15 bg-[#211a10] p-3 text-left text-xs font-medium leading-5 text-stone-300 transition-opacity duration-150 ${open ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}
        style={position}
      >
        {children}
      </span>
