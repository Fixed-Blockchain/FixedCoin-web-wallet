...
const FIX_HRP: &str = "fix";
const FIX_P2PKH_PREFIX: u8 = 0x01;
const FIX_P2SH_PREFIX: u8 = 0x00;
...
#[cfg(target_arch = "wasm32")]
#[link(wasm_import_module = "fixedcoin_crypto")]
unsafe extern "C" {
...
fn address_material(
    ...
    let (address, script, redeem_script_hex, tap_internal_key_hex) = match script_type {
        ScriptType::P2pkh => {
            ...
            (
                base58_address(FIX_P2PKH_PREFIX, public_key_hash.as_byte_array()),
                script,
                None,
                None,
            )
        ...
        ScriptType::P2shP2wpkh => {
            ...
            (
                base58_address(FIX_P2SH_PREFIX, script_hash.as_byte_array()),
                script,
        ...
        ScriptType::P2wpkh => {
            ...
            let hrp =
                Hrp::parse(FIX_HRP).map_err(|error| CryptoError::Crypto(error.to_string()))?;
...
fn parse_private_key(input: &str) -> Result<ParsedPrivateKey, CryptoError> {
    ...
    if private_key.network != NetworkKind::Main {
        return Err(CryptoError::InvalidPrivateKey(
            "WIF does not use the FixedCoin mainnet prefix".into(),
        ));
    }
...
#[cfg(target_arch = "wasm32")]
#[unsafe(no_mangle)]
pub extern "C" fn fixedcoin_wallet_crypto_alloc(length: usize) -> *mut u8 {
...
#[cfg(target_arch = "wasm32")]
#[unsafe(no_mangle)]
/// Releases a byte buffer returned by [`fixedcoin_wallet_crypto_alloc`].
///
/// # Safety
///
/// `pointer` and `length` must identify one live allocation returned by
/// [`fixedcoin_wallet_crypto_alloc`].
pub unsafe extern "C" fn fixedcoin_wallet_crypto_free_bytes(pointer: *mut u8, length: usize) {
...
/// [`fixedcoin_wallet_crypto_free`].
pub unsafe extern "C" fn fixedcoin_wallet_crypto_invoke(
...
#[unsafe(no_mangle)]
/// Releases a string returned by [`fixedcoin_wallet_crypto_invoke`].
///
/// # Safety
///
/// `value` must either be null or a pointer returned by [`fixedcoin_wallet_crypto_invoke`] that has
/// not already been released.
pub unsafe extern "C" fn fixedcoin_wallet_crypto_free(value: *mut c_char) {
    if !value.is_null() {
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_network_fixedcoin_wallet_nativecore_FixedCoinWalletCryptoModule_nativeInvoke<
    'local,
>(
...
mod tests {
    ...
    fn derives_web_wallet_primary_address() {
        ...
        assert_eq!(
            ...
            Some("fix1qcr8te4kr609gcawutmrza0j4xv80jy8zr9mwjd")
        );
        assert_eq!(
            response.as_array().unwrap()[0]
                .get("scriptType")
    ...
    fn derives_all_supported_addresses_from_compressed_wif_and_hex() {
        ...
        let expected = [
            ("p2pkh", "b1sYGBu5FKdxkL5FCSeJygmkchViYCqAq"),
            ("p2sh-p2wpkh", "1JEKB1HSaTB8d2fr528v3URQVm5ajMZviw"),
            ("p2wpkh", "fix1qw508d6qejxtdg4y5r3zarvary0c5xw7k7dgkay"),
        ];
    ...
    fn uncompressed_wif_covers_legacy_and_compressed_segwit_families() {
        ...
        assert_eq!(
            ...
            Some("dcyZChM2AVnjoP3MUUSFMKqMFnej487Nz")
        );
        assert_eq!(
    ...
    fn derivation_policy_covers_legacy_depth_but_rejects_other_paths() {
        ...
        assert_eq!(
            allowed.as_array().unwrap()[0]
                .get("address")
                .and_then(Value::as_str),
            Some("fix1qj0vycncf27q0478janvcexrltgp3lnau9l2dnh")
        );
