...
The wallet accepts 12-word and 24-word BIP39 phrases, FIX mainnet WIF keys, 32-byte hexadecimal private keys, and deterministic email/password pairs. XPRV input is intentionally unsupported.
...
Generated recovery phrases are shown only on the backup screen. Moving to verification removes the phrase from the interface and returning to that screen is not possible. During both backup and word verification, switching tabs or windows uses the background delay selected in Settings (30 seconds by default, or immediate when set to 0). Returning before that delay preserves the current step; exceeding it terminates the Worker and returns to wallet access. Closing or navigating away from the page still destroys the session immediately. An open wallet also locks on its configured inactivity and background limits.
