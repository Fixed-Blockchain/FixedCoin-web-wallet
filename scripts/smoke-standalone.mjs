...
const smokeRoot = mkdtempSync(
  join(temporaryRoot, 'fixedcoin-wallet-standalone-smoke-'),
);
...
try {
  ...
  if (
    !/<link\b[^>]*\brel=["'](?:shortcut )?icon["'][^>]*fixedcoin-logo\.png/u.test(
      rootHtml,
    )
  ) {
    throw new Error('The standalone HTML does not declare the FixedCoin PNG favicon.');
  }
  ...
  const wasmResponse = await fetch(
    `${origin}/wasm/fixedcoin_wallet_crypto_web.wasm`,
  );
  ...
  const wasmBaseline = JSON.parse(
    readFileSync(
      resolve('native', 'fixedcoin-wallet-crypto-web', 'wasm-checksum.json'),
      'utf8',
    ),
  );
  ...
  if (
    workerResponse.headers.get('cross-origin-embedder-policy') !== 'require-corp' ||
    workerResponse.headers.get('cross-origin-resource-policy') !== 'same-origin'
  ) {
    throw new Error('The cryptographic Worker does not preserve document isolation.');
  }
  const workerPolicy =
  ...
  // The standalone Worker must allow its same-origin WASM without requiring
  // a reverse proxy to supply the browser isolation and CSP headers.
  if (
    !workerPolicy ||
  ...
  if (!workerBody.includes('/wasm/fixedcoin_wallet_crypto_web.wasm')) {
    throw new Error(
