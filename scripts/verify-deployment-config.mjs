...
const nginx = readFileSync(
  resolve('deploy', 'nginx', 'fixedcoin-wallet.conf'),
  'utf8',
);
const nginxBootstrap = readFileSync(
  resolve('deploy', 'nginx', 'fixedcoin-wallet.bootstrap.conf'),
  'utf8',
);
const systemd = readFileSync(
  resolve('deploy', 'systemd', 'fixedcoin-wallet.service'),
  'utf8',
);
...
requireFragments('Nginx vhost', nginx, [
  'server_name web.wallet.fixedcoin.org;',
  'proxy_pass http://127.0.0.1:8787;',
  'Content-Security-Policy',
  'map $upstream_http_content_security_policy $fixedcoin_wallet_csp',
  'default $upstream_http_content_security_policy',
  "script-src 'self' 'wasm-unsafe-eval'",
  "connect-src 'self' wss://electrumx.fixedcoin.org:50004",
  'add_header Content-Security-Policy $fixedcoin_wallet_csp always',
...
requireFragments('Systemd unit', systemd, [
  'User=fixedcoin-wallet',
  'Group=fixedcoin-wallet',
