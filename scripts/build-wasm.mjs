...
const crateRoot = join(projectRoot, 'native', 'fixedcoin-wallet-crypto-web');
...
const wasmSource = join(
  ...
  'release',
  'fixedcoin_wallet_crypto_web.wasm',
);
const wasmOutputDirectory = join(projectRoot, 'public', 'wasm');
const wasmOutput = join(wasmOutputDirectory, 'fixedcoin_wallet_crypto_web.wasm');
...
if (reviewedChecksum.schemaVersion !== 1 || reviewedChecksum.sha256 !== checksum) {
  throw new Error(
    `WASM output drifted from the reviewed checksum (${checksum}). Review the native diff and update native/fixedcoin-wallet-crypto-web/wasm-checksum.json explicitly.`,
  );
}
writeFileSync(checksumOutput, `${checksum}  fixedcoin_wallet_crypto_web.wasm\n`, 'utf8');
