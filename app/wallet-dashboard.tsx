...
import { fixedcoinTransactionExplorerUrl } from '@/src/network/explorer';
import {
...
import { formatFixedCoinAmount } from '@/src/ui/formatFixedCoin';
import {
...
const INPUT_CLASS =
  'mt-2 min-h-12 w-full rounded-2xl border border-amber-200/[0.12] bg-[#1b160e] px-4 py-3 text-sm text-stone-100 outline-none transition-all duration-200 placeholder:text-stone-600 hover:border-amber-200/20 hover:bg-[#211a10] focus:border-amber-300/55 focus:ring-4 focus:ring-amber-300/10';
...
const networkTone = (state?: WalletNetworkState) => {
  if (
    ...
    return 'bg-emerald-300 text-stone-950';
  }
...
export function WalletDashboard({
  ...
  async function reserveNewAddress() {
    ...
    setNotice(undefined);
    try {
      let knownAddresses = snapshot.addresses;
      while (addressOperationRef.current === operation) {
        ...
        if (addressOperationRef.current !== operation) return;
        if (refreshed?.status !== 'ready' || !refreshed.record) {
          setLocalError(t('receive.syncRequired'));
          return;
        }
        knownAddresses = refreshed.record.snapshot.addresses;
        const checked = knownAddresses.find(
          (address) => address.address === issued.address,
        );
        if (!checked) {
          setLocalError(t('receive.syncRequired'));
          return;
        }
        if (checked.used) continue;
        setIssuedAddress(issued);
        setNotice(
  ...
  const recipientOutputs = (targetIndex?: number): TransparentSendOutput[] =>
    recipients.map((recipient, index) => ({
      ...
      amountSats:
        index === targetIndex
          ? BigInt(0)
          : parseFixedCoinAmountToSats(recipient.amount),
    }));
  ...
  async function ensureSendChange(outputs: readonly TransparentSendOutput[]) {
    ...
    const changeTemplate = HD_ACCOUNT_TEMPLATES.find(
      (template) => template.key === accountKey,
    )!;
    const controller = controllerRef.current;
    if (!controller) {
    ...
    if (!summary.hd) {
      const change = summary.primaryAddresses.find(
  ...
  return (
    <div className="space-y-5">
      ...
      <nav
        ...
        className="grid gap-1 rounded-3xl border border-amber-200/[0.10] bg-[#20190d] p-1.5 sm:gap-1.5"
        style={{
          ...
          <CardContent className="px-5 pb-5">
            <div className="divide-y divide-white/[0.06] text-sm">
              <div className="flex flex-col items-stretch gap-2 py-3 first:pt-0 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                <label className="text-stone-500" htmlFor="auto-lock-minutes">
                  {t('settings.lock')}
                </label>
                <select
                  id="auto-lock-minutes"
                  aria-label={t('settings.lock')}
                  className="min-h-10 w-full rounded-xl border border-white/10 bg-[#1b160e] px-3 text-sm font-semibold text-stone-100 outline-none transition hover:border-amber-200/20 focus:border-amber-300/50 focus:ring-4 focus:ring-amber-300/10 sm:w-auto"
        ...
        <Card className="overflow-visible border border-amber-300/15 bg-amber-300/[0.025] py-0">
          ...
          <CardContent className="px-5 pb-5">
            ...
            !backupRevealRequested ? (
              ...
              <form
                className="rounded-2xl border border-amber-300/15 bg-stone-950/55 p-4"
                onSubmit={(event) => {
                  event.preventDefault();
                ...
                <label className="text-xs font-semibold text-stone-300">
                  ...
                  <span className="relative mt-2 block">
                    ...
                    <button
                      ...
                      className="absolute inset-y-0 right-1 grid w-10 place-items-center rounded-lg text-stone-500 transition hover:bg-white/[0.05] hover:text-stone-200"
                      onClick={() =>
              ...
              <div className="rounded-xl border border-amber-300/20 bg-stone-950/65 p-4">
                <ol className="grid gap-2 sm:grid-cols-3 lg:grid-cols-4">
                  {revealedMnemonic.mnemonic.split(' ').map((word, index) => (
                    <li
                      key={`${index}-${word}`}
                      className="rounded-lg border border-white/8 bg-stone-950/75 px-3 py-2 font-mono text-sm text-stone-100"
                    >
                      <span className="mr-2 text-[10px] text-stone-600">
                        {index + 1}
                      </span>
                      {word}
                    </li>
      ...
      {activeView === 'home' ? (
        <div className="grid gap-5">
          <Card className="glass-panel subtle-shine relative border-amber-300/15 bg-transparent py-0">
              ...
              <div className="text-center">
                ...
                {snapshot.spendableSats !== snapshot.balanceSats ? (
                  <p className="mt-3 text-xs text-stone-500">
                    {t('home.spendable', {
                      amount: formatFixedCoinAmount(snapshot.spendableSats),
                    })}
                  </p>
                ) : null}
                {snapshot.unconfirmedSats > 0 ? (
                  <p className="mt-2 text-xs font-semibold text-amber-200">
                    {t('home.pending', {
                      amount: formatFixedCoinAmount(snapshot.unconfirmedSats),
                    })}
                  </p>
                ) : null}
            ...
            <Card className="glass-panel overflow-visible border-violet-300/15 bg-transparent py-0">
              <CardContent className="p-5 sm:p-6">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <Badge
                    variant="outline"
                    className="border-white/10 text-stone-500"
                  >
                  ...
                  <div className="mt-4 rounded-xl border border-violet-300/20 bg-violet-300/[0.035] p-4">
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <SendMetric
                        ...
                        value={`${formatFixedCoinAmount(consolidationPreview.plan.totalFeeSats)} FIX`}
                      />
                    </div>
                    {!preparedConsolidation ? (
          ...
          <Card className="glass-panel border-amber-300/15 bg-transparent py-0">
            <CardHeader className="px-5 pt-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <InfoTip label={t('receive.infoLabel')}>
                      {summary.hd ? (
                        <>
                          {t('receive.hdInfo')}
                          {receivePath ? (
                            <span className="mt-2 block font-mono text-[10px] text-stone-500">
                ...
                {familyFor(selectedFamily).preferred ? (
                  <Badge className="bg-amber-200 text-stone-950">
            ...
            <CardContent className="px-5 pb-5">
              {summary.hd ? (
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {HD_ACCOUNT_TEMPLATES.map((family) => (
                    <button
                      ...
                      className={`group/family min-h-14 rounded-2xl border px-3 py-2 text-left text-xs font-bold transition-all duration-200 hover:-transtone-y-0.5 ${
                        selectedFamily === family.key
                          ? 'border-amber-300/40 bg-amber-300/12 text-amber-100'
                          : 'border-white/8 bg-stone-950/45 text-stone-400 hover:border-white/15 hover:bg-white/[0.055] hover:text-stone-200'
                      }`}
                    >
                      <span className="block font-bold">{family.label}</span>
                      {family.preferred ? (
                        <span className="mt-1 block text-[10px] font-semibold text-amber-300/75">
                          {t('receive.recommended')}
                        </span>
                      ) : null}
                    </button>
                ...
                <div className="rounded-xl border border-amber-300/20 bg-amber-300/[0.06] px-3 py-2 text-xs leading-5 text-amber-100">
                  {t('receive.singleKeyInfo')}
                </div>
              )}

              <div className="mt-5 min-h-28 rounded-3xl border border-white/8 bg-stone-950/60 p-4 sm:p-5">
                {addressBusy ? (
                  <div className="flex min-h-20 items-center justify-center gap-2 text-sm text-stone-400">
                  ...
                  <div className="grid items-center gap-4 sm:grid-cols-[minmax(0,1fr)_10rem]">
                    <div>
                      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-stone-400">
                        {familyFor(selectedFamily).label}
                        {receiveIndex !== undefined
                          ? t('receive.indexSuffix', { index: receiveIndex })
                          : null}
                      </p>
                    ...
                    >
                      ...
                      />
                      <path d={receiveQr.path} fill="#110e09" />
                    </svg>
                  </div>
                ) : (
                  <p className="flex min-h-20 items-center justify-center text-sm text-stone-500">
      ...
      {activeView === 'settings' && summary.hd ? (
        <Card className="border border-white/8 bg-card/65 py-0">
          <details className="group/details">
            <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 outline-none transition-colors hover:bg-white/[0.025] focus-visible:ring-4 focus-visible:ring-inset focus-visible:ring-amber-300/10">
              <div>
                <CardTitle className="text-base text-white">
              ...
              <ChevronDown
                className="size-4 text-stone-500 transition-transform duration-200 group-open/details:rotate-180"
                aria-hidden="true"
              />
            </summary>
                ...
                <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                  {coverage.map((item) => (
                    <div
                      ...
                      className="grid grid-cols-[1fr_auto] gap-3 rounded-xl border border-white/8 bg-stone-950/45 p-3"
                    >
                      <div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.045] px-4 py-2.5 text-right">
                ...
                <p className="mt-1 font-mono text-sm font-bold text-emerald-100">
                  {formatFixedCoinAmount(snapshot.spendableSats)} FIX
                </p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="px-5 pb-5">
            ...
            >
              {recipients.map((recipient, index) => (
                <div
                  key={recipient.id}
                  className="grid gap-3 rounded-2xl border border-white/8 bg-stone-950/45 p-3.5 lg:grid-cols-[minmax(0,1fr)_14rem_auto]"
                >
                  <label className="text-xs font-semibold text-stone-300">
                  ...
                  <label className="text-xs font-semibold text-stone-300">
                    ...
                    <div className="relative">
                      ...
                      {shouldOfferMaxForRecipient(
                        ...
                        <button
                          type="button"
                          className="absolute right-2 top-1/2 mt-1 -transtone-y-1/2 rounded-md px-2 py-1 text-[10px] font-bold text-amber-200 hover:bg-amber-300/10 disabled:cursor-not-allowed disabled:opacity-40"
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="w-full max-w-sm rounded-xl border border-white/8 bg-stone-950/35 px-3 py-2.5 text-xs sm:w-auto sm:min-w-72">
                  ...
                  <details className="mt-2 border-t border-white/[0.06] pt-2">
                    <summary className="cursor-pointer select-none font-semibold text-stone-500 hover:text-stone-300">
                      {t('send.advanced')}
                    </summary>
                    <label className="mt-3 flex cursor-pointer items-start gap-2 text-stone-400">
                      <input
                    ...
                    {customFeeEnabled ? (
                      <label className="mt-3 block font-semibold text-stone-300">
                        {t('send.customFee')}
                        <input
                          className={INPUT_CLASS}
                          value={feeRate}
            ...
            {sendPreview ? (
              <div className="mt-4 rounded-2xl border border-amber-300/20 bg-amber-300/[0.035] p-4">
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <SendMetric
                  ...
                  <SendMetric
                    ...
                    value={`${formatFixedCoinAmount(sendPreview.estimate.amountSats)} FIX`}
                  />
                  <SendMetric
                    label={t('send.metricFee')}
                    value={`${formatFixedCoinAmount(sendPreview.estimate.feeSats)} FIX`}
                  />
                  <SendMetric
              ...
              <div className="mt-4 rounded-2xl border border-emerald-300/25 bg-emerald-300/[0.04] p-4">
                ...
                <p className="mt-2 text-xs leading-5 text-stone-500">
                  {t('send.signedSummary', {
                    ...
                    fee: formatFixedCoinAmount(preparedSend.feeSats),
                  })}
                </p>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Button
          ...
          <Card className="border border-violet-300/15 bg-violet-300/[0.025] py-0">
            <details className="group/search">
              <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 outline-none transition-colors hover:bg-white/[0.025] focus-visible:ring-4 focus-visible:ring-inset focus-visible:ring-violet-300/10">
                ...
                <ChevronDown
                  className="size-4 text-stone-500 transition-transform duration-200 group-open/search:rotate-180"
                  aria-hidden="true"
                />
              </summary>
              <CardContent className="border-t border-white/[0.06] px-5 pt-4 pb-5">
...
function TransactionBroadcastDialog({
  ...
  const displayedTxid = replacementFailed
    ...
    : receipt.txid;
  const explorerUrl = fixedcoinTransactionExplorerUrl(displayedTxid);
  ...
  return (
    ...
    >
      <dialog
        ...
        aria-modal="true"
        className="relative m-0 max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-[2rem] border border-amber-200/[0.14] bg-[#231b10] p-0 text-stone-100"
      >
        ...
        <div className="space-y-4 px-5 py-5 sm:px-6 sm:py-6">
          <div className="rounded-2xl border border-white/[0.08] bg-stone-950/55 p-4">
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-stone-500">
              {t('transaction.id')}
            </p>
            <a
              className="mt-2 flex items-start gap-2 break-all font-mono text-xs leading-5 text-amber-300 underline decoration-amber-300/30 underline-offset-4 transition hover:text-amber-200 hover:decoration-amber-200 focus-visible:rounded-md focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-300/15"
              href={explorerUrl}
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.018] p-4">
            ...
            <div
              aria-hidden="true"
              className="h-2 overflow-hidden rounded-full bg-stone-950/80"
            >
              <div
                className={`h-full rounded-full bg-gradient-to-r transition-[width] duration-700 ${replacementFailed ? 'from-amber-500 via-amber-300 to-amber-400' : 'from-amber-500 via-amber-300 to-emerald-300'} ${progressWidth}`}
              />
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2 text-[10px] font-bold uppercase tracking-[0.1em]">
              ...
              <span className="text-center text-amber-200">
                {t('transaction.progressMempool')}
              </span>
              <span
                className={`text-right transition-colors ${replacementFailed ? 'text-amber-200' : isConfirmed ? 'text-emerald-200' : 'text-stone-600'}`}
              >
          ...
          {isReplacement ? (
            <output
              className={`block rounded-2xl border p-3 text-xs leading-5 ${replacementFailed ? 'border-amber-300/20 bg-amber-300/[0.05] text-amber-100/85' : isConfirmed ? 'border-emerald-300/20 bg-emerald-300/[0.05] text-emerald-100/85' : 'border-amber-300/20 bg-amber-300/[0.04] text-amber-100/85'}`}
            >
              <p>
                {replacementFailed
            ...
            <div className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.04] p-4">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <SendMetric
                  ...
                  value={`${formatFixedCoinAmount(quote.originalFeeSats)} FIX`}
                />
                <SendMetric
                  label={t('transaction.replacementFee')}
                  value={`${formatFixedCoinAmount(quote.feeSats)} FIX`}
                />
              </div>
...
function ManualSyncCooldownDialog({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#120f0a]/80 p-4 backdrop-blur-sm">
      <dialog
        aria-labelledby="manual-sync-cooldown-title"
        aria-modal="true"
        className="glass-panel relative m-0 max-h-[calc(100dvh-2rem)] w-full max-w-sm overflow-y-auto rounded-3xl border border-amber-200/[0.12] p-0 text-stone-100"
        open
      >
        <div className="p-6">
          <div className="flex items-start gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-2xl border border-amber-300/15 bg-amber-300/[0.08] text-amber-200">
    ...
    <div className="flex min-h-[calc(100dvh-8rem)] items-center justify-center py-10">
      <div className="w-full max-w-md text-center">
        ...
        <div className="mt-7 overflow-hidden rounded-full bg-white/[0.07]">
          <div
            className={`h-1.5 rounded-full transition-[width] duration-500 ${failed ? 'bg-red-400' : 'bg-[#e9a012]'}`}
...
function SendMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/8 bg-stone-950/45 p-3">
      <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-stone-600">
        {label}
      </p>
      <p className="mt-2 font-mono text-sm font-bold text-stone-100">{value}</p>
    </div>
  );
}
